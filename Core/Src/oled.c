...
uint8_t OLED_GRAM[OLED_SIZE_WIDTH + 16][OLED_SIZE_PAGES];
...
void OLED_WR_Byte(uint8_t dat, uint8_t mode)
{
    ...
    I2C_Error_Check(HAL_I2C_Master_Transmit(&hi2c1, OLED_ADDR, sendbyte.sendbyte, sizeof(sendbyte), 10));
}
...
{
    uint8_t data[OLED_SIZE_COLUMNS];
    uint8_t i, n;
    for (i = 0; i < OLED_SIZE_PAGES; i++)
    {
        OLED_WR_Byte(0xb0 + i, OLED_CMD); // 设置行起始地址
        OLED_WR_Byte(0x00, OLED_CMD);	  // 设置低列起始地址
        OLED_WR_Byte(0x10, OLED_CMD);	  // 设置高列起始地址

        for (n = 0; n < OLED_SIZE_COLUMNS; n++)
        {
            data[n] = OLED_GRAM[n][i];
        }

        I2C_Error_Check(HAL_I2C_Mem_Write(&hi2c1, OLED_ADDR, 0x40, 1, (uint8_t *)data, sizeof(data), 50));
    }
}
...
void OLED_ScrollDisplay(uint8_t num, uint8_t space, uint8_t mode)
{
    ...
    while (1)
    {
        if (m == 0)
        {
            OLED_ShowChinese(128, 16, t, 16, mode); // 写入一个汉字保存在OLED_GRAM[][]数组中
            t++;
        }
        if (t == num)
        {
...
void OLED_Init(void)
{
    ...
    OLED_WR_Byte(0xE3, OLED_CMD); /*nop*/
...
#if OLED_SIZE_HEIGHT == 64
    OLED_WR_Byte(0xA8, OLED_CMD); /*multiplex ratio*/
	OLED_WR_Byte(0x3F, OLED_CMD); /*duty = 1/32*/
...
struct
{
	...
	uint16_t auiDisplayCache[OLED_SIZE_WIDTH][OLED_SIZE_PAGES];
} s_stLCDBuffer;
...
void SSD1306_WR_Byte(uint8_t dat, uint8_t mode)
{
	...
	I2C_Error_Check(HAL_I2C_Master_Transmit(&hi2c1, OLED_ADDR, data, 2, 10));
}
/*****************************************************************************/
...
void SSD1306_SetPosition(uint8_t uiColumn, uint8_t uiPage)
{
	...
	sendbyte.SSD1306IIC.mode = OLED_CMD;
...
void SCREEN_ClearDisplayBuffer(void)
{
	...
	for (uiCurrentPageIndex = 0; uiCurrentPageIndex < OLED_SIZE_PAGES; uiCurrentPageIndex++)
	{
		for (uiCurrentColumnIndex = 0; uiCurrentColumnIndex < OLED_SIZE_WIDTH; uiCurrentColumnIndex++)
		{
			s_stLCDBuffer.auiDisplayCache[uiCurrentColumnIndex][uiCurrentPageIndex] = 0x00;
		}
	}
	s_stLCDBuffer.stUpdateArea.uiStartPageIndex = (OLED_SIZE_HEIGHT / 8) - 1;
	s_stLCDBuffer.stUpdateArea.uiEndPageIndex = 0;
	s_stLCDBuffer.stUpdateArea.uiStartColumnIndex = OLED_SIZE_WIDTH - 1;
...
void SCREEN_SetPixel(int iPosX, int iPosY, int iColor)
{
	...
	if ((iPosX < OLED_SIZE_WIDTH) && (iPosY < OLED_SIZE_HEIGHT))
	{
		// Check and set page and column index.
...
void SCREEN_RefreshScreen(void)
{
	...
	uint8_t data[OLED_SIZE_WIDTH], index;

	if (s_stLCDBuffer.stUpdateArea.uiEndColumnIndex > OLED_SIZE_WIDTH - 1)
	{
		s_stLCDBuffer.stUpdateArea.uiEndColumnIndex = OLED_SIZE_WIDTH - 1;
	}
	if (s_stLCDBuffer.stUpdateArea.uiEndPageIndex > (OLED_SIZE_HEIGHT / 8) - 1)
	{
		s_stLCDBuffer.stUpdateArea.uiEndPageIndex = (OLED_SIZE_HEIGHT / 8) - 1;
	}
	...
	while (uiChangedPageIndex <= s_stLCDBuffer.stUpdateArea.uiEndPageIndex)
	{
		...
		I2C_Error_Check(HAL_I2C_Mem_Write(&hi2c1, OLED_ADDR, OLED_DATA, 1, (uint8_t *)data, index, 50));
